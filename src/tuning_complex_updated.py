...
def train_model(
    ...
    # ==================================================================
    # WEIGHT FOR FIRST TOKEN (ONLY CHANGE)
    # ==================================================================
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch in tqdm(train_loader, desc=f"Train Epoch {epoch+1}"):
            ...
            # Extract features: [B, 1, 1536] → squeeze to [B, D]
            ...
            # ==================================================================
            # WEIGHTED DECODER LOSS (FIRST TOKEN HAS HIGHER WEIGHT)
            # ==================================================================
            ce_loss_per_token = F.cross_entropy(
                logits.view(-1, logits.size(-1)), 
                full_targets_seq.view(-1), 
                ignore_index=pad_token_id, 
                reduction='none'
            ).view(10 * B, max_seq_len)
            
            token_weights = torch.ones_like(ce_loss_per_token)
            token_weights[:, 0] = first_token_weight
            
            weighted_decoder_loss = (ce_loss_per_token * token_weights).mean()
            ...
            # Total loss
            total_loss = weighted_decoder_loss + contrastive_lambda * contrastive_loss
            ...
            train_loss += weighted_decoder_loss.item() * (10 * B)
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Val Epoch {epoch+1}"):
                ...
                val_loss += weighted_decoder_loss.item() * (10 * B)

...
def train_model(
    ...
    if config['training'].get('contrastive_regularizer', False):
        regularizer = torch.nn.CosineEmbeddingLoss(margin=0.5)
        contrastive_lambda = 0.05
        print(f"[Config] Contrastive Regularizer enabled (lambda={contrastive_lambda}, margin=0.5)")
    else:
        regularizer = None
        contrastive_lambda = 0.0
        print(f"[Config] Contrastive Regularizer disabled")
    # ------------------------------
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for orig_batch, aug_batch, idx_batch in tqdm(train_loader, desc=f"Train Epoch {epoch+1}"):
            ...
            if regularizer is not None:
                # Pool features to vectors [B*B, D] (mean over sequence length)
                orig_vectors = orig_all_features.mean(dim=1) 
                aug_vectors = aug_all_features.mean(dim=1)
                
                # Convert labels {0, 1} -> {-1, 1} for CosineEmbeddingLoss
                contrastive_labels = 2 * target_classes - 1 
                
                contrastive_loss_val = regularizer(orig_vectors, aug_vectors, contrastive_labels)
            
            total_loss = decoder_loss + contrastive_lambda * contrastive_loss_val
            # ------------------------------------
            ...
            n_pairs = B * B
            train_loss += decoder_loss.item() * n_pairs
            train_contrastive_loss += contrastive_loss_val.item() * n_pairs
            total_pairs += n_pairs

        lr_scheduler.step()

        avg_train_loss = train_loss / total_pairs
        avg_train_contrastive = train_contrastive_loss / total_pairs
        ...
        with torch.no_grad():
            for orig_batch, aug_batch, idx_batch in tqdm(val_loader, desc=f"Val Epoch {epoch+1}"):
                ...
                n_pairs = B * B
                val_loss += decoder_loss.item() * n_pairs
                val_contrastive_loss += contrastive_loss_val.item() * n_pairs
                val_total_pairs += n_pairs

        avg_val_loss = val_loss / val_total_pairs
        avg_val_contrastive = val_contrastive_loss / val_total_pairs
        ...
        print(f"  Train Loss: {avg_train_loss:.4f} | Contr: {avg_train_contrastive:.4f} | SeqAcc: {avg_train_seq_acc:.4f} | TokAcc: {avg_train_token_acc:.4f}")
        print(f"  Train Prec: {train_precision:.4f} | Recall: {train_recall:.4f} | F1: {train_f1:.4f}")
        print(f"  Val   Loss: {avg_val_loss:.4f} | Contr: {avg_val_contrastive:.4f} | SeqAcc: {avg_val_seq_acc:.4f} | TokAcc: {avg_val_token_acc:.4f}")
        print(f"  Val   Prec: {val_precision:.4f} | Recall: {val_recall:.4f} | F1: {val_f1:.4f}")
        print(f"  LR: {current_lr:.2e}")

        writer.add_scalar("Loss/Train", avg_train_loss, epoch)
        writer.add_scalar("Loss/Contrastive/Train", avg_train_contrastive, epoch)
        writer.add_scalar("SeqAcc/Train", avg_train_seq_acc, epoch)
        writer.add_scalar("TokAcc/Train", avg_train_token_acc, epoch)
        
        writer.add_scalar("Loss/Val", avg_val_loss, epoch)
        writer.add_scalar("Loss/Contrastive/Val", avg_val_contrastive, epoch)

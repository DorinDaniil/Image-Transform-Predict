...
# Dictionary of available transformations and their corresponding IDs
TRANSFORM_TOKENS = {
...
class ImageTransformer:
    ...
    def __init__(self):
        ...
        """
        self.transformations = list(TRANSFORM_TOKENS.keys())
    ...
    def resize(self, image: Image.Image) -> Image.Image:
        ...
        if random.random() > 0.5:
            # Option 1: Uniform resize
            scale = random.uniform(0.3, 2)
            w, h = image.size
            new_w, new_h = int(w * scale), int(h * scale)
        else:
            # Option 2: Non-uniform resize
    ...
    def sample_transformations(self, p: float = 0.3) -> List[str]:
        """
        Sample a random sequence of transformations, ensuring rotations are mutually exclusive.

        Args:
            p (float): Probability of selecting each transformation (default: 0.3).
